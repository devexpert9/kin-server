...
exports.create_user_admin = function(req, res) {
  superadmin.findOne({email: req.body.email}, function(err, adminuser) {
    if(adminuser == null)
    {
...
exports.forgot_password_admin = function(req, res) {
    superadmin.findOne({email: req.body.email}, function(err, user) {
      if(user){
         var numsms = Math.floor(Math.random() * 90000) + 10000;
          superadmin.update({_id: user._id }, { $set: { otp: numsms}}, {new: true}, function(err, task) {
          if (task == null){
            res.send({
              error: err,
              status: 0,
...
exports.otp_verification = function(req, res) {
  superadmin.findOne({_id: req.body._id, otp: req.body.otp }, function(err, otp) {
    if (otp == null){
      res.send({
        error: err,
        status: 0,
...
exports.update_admin_password = function(req, res) {
  console.log(req.body);
   superadmin.findOne({username: req.body.email}, function(err, doc) {
    console.log(doc)
    if (doc == null){
          res.send({
            error: err,
      ...
      if(doc.password == req.body.oldpassword){
        superadmin.update({_id: doc._id }, { $set: { password: req.body.newpassword}}, {new: true}, function(err, change) {
        if (change == null){
          res.send({
            error: err,
            status: 0,
